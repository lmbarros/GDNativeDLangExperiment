...
public:
	...
	{
		static assert(staticIndexOf!(StreamTexture, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit StreamTexture");
	...
	{
		static assert(is(typeof(T.owner) : StreamTexture) || staticIndexOf!(StreamTexture, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend StreamTexture");
	...
	int load(in String path)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("StreamTexture", "load");
	...
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("StreamTexture", "get_load_path");

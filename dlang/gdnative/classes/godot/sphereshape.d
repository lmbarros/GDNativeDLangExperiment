...
public:
	...
	{
		static assert(staticIndexOf!(SphereShape, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit SphereShape");
	...
	{
		static assert(is(typeof(T.owner) : SphereShape) || staticIndexOf!(SphereShape, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend SphereShape");
	...
	void set_radius(in float radius)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("SphereShape", "set_radius");
	...
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("SphereShape", "get_radius");

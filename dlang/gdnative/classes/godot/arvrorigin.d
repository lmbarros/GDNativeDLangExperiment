...
public:
	...
	{
		static assert(staticIndexOf!(ARVROrigin, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit ARVROrigin");
	...
	{
		static assert(is(typeof(T.owner) : ARVROrigin) || staticIndexOf!(ARVROrigin, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend ARVROrigin");
	...
	void set_world_scale(in float world_scale)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("ARVROrigin", "set_world_scale");
		const(void*)[1] _GODOT_args = [cast(void*)(&world_scale), ];
		godot_method_bind_ptrcall(mb, cast(godot_object)(this), _GODOT_args.ptr);
	}
	float get_world_scale() const
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("ARVROrigin", "get_world_scale");

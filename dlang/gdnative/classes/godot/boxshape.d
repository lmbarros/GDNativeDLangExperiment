...
public:
	...
	{
		static assert(staticIndexOf!(BoxShape, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit BoxShape");
	...
	{
		static assert(is(typeof(T.owner) : BoxShape) || staticIndexOf!(BoxShape, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend BoxShape");
	...
	void set_extents(in Vector3 extents)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("BoxShape", "set_extents");
	...
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("BoxShape", "get_extents");

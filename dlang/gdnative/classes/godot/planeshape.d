...
public:
	...
	{
		static assert(staticIndexOf!(PlaneShape, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit PlaneShape");
	...
	{
		static assert(is(typeof(T.owner) : PlaneShape) || staticIndexOf!(PlaneShape, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend PlaneShape");
	...
	void set_plane(in Plane plane)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("PlaneShape", "set_plane");
		const(void*)[1] _GODOT_args = [cast(void*)(&plane), ];
		godot_method_bind_ptrcall(mb, cast(godot_object)(this), _GODOT_args.ptr);
	}
	Plane get_plane() const
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("PlaneShape", "get_plane");

...
public:
	...
	{
		static assert(staticIndexOf!(PinJoint, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit PinJoint");
	...
	{
		static assert(is(typeof(T.owner) : PinJoint) || staticIndexOf!(PinJoint, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend PinJoint");
	...
	void set_param(in int param, in float value)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("PinJoint", "set_param");
	...
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("PinJoint", "get_param");

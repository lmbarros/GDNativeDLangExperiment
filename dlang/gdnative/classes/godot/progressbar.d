...
public:
	...
	{
		static assert(staticIndexOf!(ProgressBar, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit ProgressBar");
	...
	{
		static assert(is(typeof(T.owner) : ProgressBar) || staticIndexOf!(ProgressBar, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend ProgressBar");
	...
	void set_percent_visible(in bool visible)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("ProgressBar", "set_percent_visible");
	...
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("ProgressBar", "is_percent_visible");

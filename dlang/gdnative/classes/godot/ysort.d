...
public:
	...
	{
		static assert(staticIndexOf!(YSort, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit YSort");
	...
	{
		static assert(is(typeof(T.owner) : YSort) || staticIndexOf!(YSort, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend YSort");
	...
	void set_sort_enabled(in bool enabled)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("YSort", "set_sort_enabled");
	...
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("YSort", "is_sort_enabled");

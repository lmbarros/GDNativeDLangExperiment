...
public:
	...
	{
		static assert(staticIndexOf!(CircleShape2D, T.BaseClasses) != -1, "Godot class "~T.stringof~" does not inherit CircleShape2D");
	...
	{
		static assert(is(typeof(T.owner) : CircleShape2D) || staticIndexOf!(CircleShape2D, typeof(T.owner).BaseClasses) != -1, "D class "~T.stringof~" does not extend CircleShape2D");
	...
	void set_radius(in float radius)
	{
		...
		if(mb is null) mb = godot_method_bind_get_method("CircleShape2D", "set_radius");
	...
	{
		static godot_method_bind* mb = null;
		if(mb is null) mb = godot_method_bind_get_method("CircleShape2D", "get_radius");
